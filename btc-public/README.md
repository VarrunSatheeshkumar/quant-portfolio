...
- Check [one cluster touch](WRITEUP.md#one-touch-from-map-state-to-measurement) by hand, then read the [continuation table](WRITEUP.md#price-after-liquidation-prints) and [forecast comparison](WRITEUP.md#the-volatility-forecast).
...
The measurements, with their intervals and what could not be established, are in [the write-up](WRITEUP.md).
...
- **Map and price response:** a liquidation map built from public open interest, price and funding on a five-minute grid; [five framings](WRITEUP.md#what-was-asked) of "does price respond to the map?", each with a hypothesis recorded privately before its result commit (the public statistics for framing 1 are not the registered statistic).
- **Volatility forecasts:** a 24-feature ridge volatility model compared against GARCH(1,1) and two HAR-RV specifications.
- **Option structures:** eight costed option structures; the write-up reports the measurements and conditional formula outputs.
