...
The 30-minute interval [−4.8, 4.0] lies inside ±10 bps; the design's MDE is 6.5 bps.

The three stored continuation estimates are:
...
53,088 prints on 61 day-blocks (47 first-of-month training days); day-block bootstrap MDE 6.5 bps.

These are the stored intervals in ledger B4–B5 and B8, with the observation-unit and day-block limitations below.
...
R² in the sealed blocks: 0.457 against 0.311 and 0.554 against 0.165 at one hour, 0.464 against 0.232 and 0.540 against −0.233 at four hours, Model A against GARCH(1,1); on QLIKE GARCH is better in three of four sealed cells.
...
The GARCH parameters coincide with arch's starting-value grid and convergence status is not preserved in the reported output. Two of Model A's inputs are fitted once on all training rows and supplied to every month. By arithmetic from the minimum-training-rows rule, the last two months of each block have no forecasts.
...
There are eight stored MDEs. Eight structures were costed; the smallest stored MDE is 41.8 bps, computed as 2.80 × SD(long − short)/√(2n) on weekly entries treated as independent.

On the 2021-onward subsample (190 weekly strips) the stored idealised MDE is 81.6 bps (137.3 on the full sample); (81.6/10)² = 67; 67 × 4.6 years = 309.
