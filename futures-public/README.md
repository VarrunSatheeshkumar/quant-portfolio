...
- On a fresh clone (no `data/`), `./reproduce.sh` runs every build stage from s0, fetching from Yahoo Finance, TreasuryDirect, Binance and EIA; s9 does not re-score the hold-out (the tracked look counter already records a scoring) and retains the tracked `RESULTS.md`; the candidate fetch, splice check and eight candidate tests then run. The tracked reports can be read without data. See [Running it](#running-it) for the command sequence and what the run produced on 2026-09-13.
...
On a fresh clone `build.py` ignores the tracked progress file when `data/grid/w0.parquet` is absent and runs every stage; at s9 the tracked look counter already records a scoring, so the hold-out is not re-scored and the tracked `RESULTS.md` and `s9.md` are retained (a second scoring is not a hold-out). The training-period stage reports (`s0`–`s8`) and the candidate reports are regenerated. The public `LOG.md` is regenerated by `reproduce.sh` and carries neither the C18 void entry nor the original timestamps.

On 2026-09-13 a fresh clone ran `./reproduce.sh` end to end in about 32 minutes (exit 0): stages s0–s8 regenerated with the same gate summaries as the tracked stage reports, s9 retained the tracked hold-out result without re-scoring, the candidate fetch took about 29 minutes, and the eight candidate tests, the C18 audit and the document test reproduced the recorded outputs. The tracked files that run rewrote: 0 stage reports (), 0 files under `reports/candidates/`, and `futures-public/reports/candidates/LOG.md`, `futures-public/reports/progress.json`; untouched: `reports/RESULTS.md`, `reports/stage_reports/s9.md`, `reports/holdout_looks.json`.
