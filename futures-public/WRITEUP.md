...
The projected edge/cost ratio is 2.3 under an assumed gross Sharpe of 0.4 and the specified cost and schedule model; the median modelled round trip is 2.7 bp, 4.1 % of the median daily dollar move over the final 250 observations.

Three signals — trend, carry and value — were specified in advance with a modelled cost and a drawdown-mandate simulator, on daily futures data from free sources across 13–27 markets. The cost ratio was projected before the signals were evaluated. It was computed without evaluating any strategy's realised return; the return panel was read to compute dollar volatility.
...
| --- | ---: | --- |
| Trend | 0.24 | Positive point estimate. |
| Carry | 0.08 | 13 covered markets; the design's mean-return MDE in Sharpe units is 0.61. |
| Value | −0.44 | Negative point estimate, opposite the predicted sign. |
...
Net Sharpe 0.06, realised vol 16.6 %; max drawdown −53 % (from the first running peak, not from initial capital); diversification ratio squared 8.6; leave-one-signal-out Sharpe differences: trend +0.45, value −0.28.

The combined book's reported median portfolio scale factor λ is 2.01.

The design's mean-return MDE in Sharpe units 0.62 / 0.61 / 0.57 / 0.60 applies to trend / carry / value / the combined programme respectively.

The public run's combined net Sharpe (0.064) exceeded its implemented 40-draw placebo threshold (0.05); the private vintage's did not (0.08).

### The hold-out

Hold-out net Sharpe −1.28 (annual return −21 %; trend −0.70, carry +0.69, value −0.72).

The figure 0.71 quoted with it is the s9 consistency check's training-based calculation, sqrt((1 + 0.5 × 0.064²)/2); no interval for the hold-out Sharpe has been computed. The s9 consistency check passes for any hold-out Sharpe within ±1.42 of the training value; the observed −1.28 lies 1.34 below it.

The block was scored 2026-09-09 (−1.18 on a partial final bar) and 2026-09-12 (−1.28 on the settled close); an aborted run had registered a look at 11:43 on 09-09 without scoring.
...
None of these futures candidates met the registered threshold.
...
For example, QTUM's long return is `(2.183 / 2.360 − 1) × 100 − 0.210000 = −7.710000%`.

The preceding book held NEO, VET, XMR, BAT and BTC long; LTC, ETH, ONT, ZEC and LINK short. Moving to this week's targets closes four longs and two shorts, opens five longs and one short, and flips NEO from +0.2 to −0.2; ETH, ONT and ZEC retain their −0.2 targets.

Absolute target-weight changes sum to 2.8 one-leg units, with drift ignored; these are not recorded execution trades.

Four names remain in the ten-name union, so the reported turnover proxy is `1 − 4/10 = 0.6`. Applying its 20 bp round-trip rate gives **0.120000% for this example**.

The report subtracts the mean of this proxy from fitted gross alpha and labels the result "net alpha"; that is the withdrawn A9 figure. Net alpha with costs entered into the weekly series has not been computed. The funding calculation has not been reconciled to payments on actual held notionals at payment timestamps.
...
Of 500 specified mandates, 53 reach a pass probability of at least 50 % at their best tested volatility target on the training path. Classifying each mandate by its most common non-pass outcome at that target: daily-loss breach 68 %, drawdown breach 28 %, timeout 4 %. Historical, overlapping starts, before the drawdown-accounting correction.

On the tested grid (5–40 %), the pass-maximising target for the example mandate is 25 % and the Calmar-maximising target is 40 %, the grid's upper endpoint; Calmar is 0.02 at every lower target to two decimals.

The historical simulator reports a 22.3 % pass rate for the example mandate at 15 % vol; the specification states an approximately 23 % synthetic prior for a Sharpe of 0.8, whose calculation has not been recovered.
...
The hold-out was scored on 2026-09-09 and 2026-09-12, with an aborted registration before the first scoring. Roll-switch offsets were selected on the full price history including the sealed period; cost normalisation uses the final 250 observations (inside the sealed period), and schedule turnover is evaluated from 2002 through the end of the series without a training cutoff. The candidate tests other than C19 use data through 2026-09-09. On a fresh clone the build regenerates the missing training-stage outputs. At s9 the tracked look counter already records a scoring, so the hold-out is not re-scored; both retained reports must exist and be nonempty, or the run stops. The README and saved reproduction record distinguish generated outputs from retained historical reports.

### Corrections to previously published figures and sentences

The full corrections list explicitly marks **10 quoted assertions as withdrawn**, counting paired assertions separately.
Three highlighted corrections: "+0.93 % per week net of a 20-basis-point round trip at 41 % weekly turnover" is withdrawn; "opened once" becomes two scorings, −1.18 and −1.28; "Breadth is the sample size" is withdrawn.
The [full corrections and counting convention](../audit/CORRECTIONS.md#futures-p8), [claim ledger](../audit/CLAIM_LEDGER.md), [audit guide](../audit/README.md) and [evidence record](../audit/EVIDENCE_RECORD.md) retain the sources, permitted interpretations and private-source limitations; coverage is not an exhaustive certification.

### What I would do differently

I would stop when a registered halt condition fired and label any subsequent work as a separate exploratory continuation.
