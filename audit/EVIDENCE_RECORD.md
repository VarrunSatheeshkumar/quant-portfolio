...
Written 2026-09-12 as "Stage 0" of a staged remediation plan. References below to the state of the public repositories describe that snapshot, before the later write-up corrections and publication of this record. The original stage numbers refer to that historical plan, not a current work queue. On 2026-09-13 the framing-5 placebo figure and unverified spill explanation, framing-2 units, and C18 target-change terminology below were aligned with the corrected ledger. Current reproduction behaviour is in [REPRODUCTION.md](REPRODUCTION.md).
...
| Framing 5: 53,088 prints, 61 days; 15m −0.94 [−3.54, 1.67] MDE 3.79; 30m −0.32 [−4.82, 4.00] MDE 6.54; 60m +3.81 [−1.48, 10.27] MDE 8.76; placebo −7.07 vs 1.1 (95th percentile of absolute deviations from the placebo mean; B6) | `event_study.json`; private v10: 66,387 prints / 63 training days, same 30m and 60m values | as above | training-masked prints with a prediction; 61 day-blocks against 47 first-of-month training days; the origin of the additional 14 blocks was not verified (E7) | **P**: numbers reproduce; three horizons registered, one quoted |
...
| Framing 2: slope −0.092 [−0.633, 0.418], MDE 0.753 (bps per unit log pressure), n = 4,036 on 70 days; notional-only control −0.055 [−0.39, 0.26] | `event_study.json`; private v3 | Tardis book snapshots, 70 days | training | **E** |
...
| C18 cost "20 bp round trip at 41 % turnover" = 0.083 %/wk | `run.py:221`: union-turnover share × 0.20 | — | recomputed from the cached book this stage: the book is 200 % gross (one leg = 1.0); mean absolute target-weight changes Σ\|Δw\| = **1.888 per week in units of one leg's notional**, with drift ignored (94 % of gross), from ≈ 20.8 entries, 20.7 exits and 2.5 flips per week across ≈ 44 names | **E**: the charged figure prices a one-leg round trip on the share of names replaced; it does not price those target-weight changes; actual rebalance trades are not reconstructed |
